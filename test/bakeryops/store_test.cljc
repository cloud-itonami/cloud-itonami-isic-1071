...
(deftest production-batch-test
  (testing "retrieve an existing batch"
    (let [batch-data {:product-type :bread/white-loaf :baking-temp-c 200}
...
(deftest log-batch-test
  (testing "logging a batch marks it as processed"
    (let [st {:batches {}}
          batch-data {:product-type :bread/white-loaf}
  ...
  (testing "logging preserves batch data"
    (let [st {:batches {}}
          batch-data {:product-type :bread/white-loaf :baking-temp-c 200}
          result (store/log-batch st "batch-001" batch-data)]
      (is (= (:product-type (get-in result [:batches "batch-001"])) :bread/white-loaf))
      (is (= (:baking-temp-c (get-in result [:batches "batch-001"])) 200)))))

;; ──────────────────────── Shipment Finalization ──────────────────────

(deftest finalize-shipment-test
  (testing "finalizing a batch marks it as finalized"
    (let [st {:batches {"batch-001" {:product-type :bread/white-loaf}}}
