(ns bakeryops.registry
  "Pure validation functions for bakery-production parameters. These are
  called by the Governor to independently verify physical/operational
  constraints -- the advisor's confidence is NOT sufficient to override
  these checks.
  ...
  reader-conditional at the call site (see `bakeryops.governor`)."
  (:require [clojure.set :as set]))

(defn baking-temp-out-of-range?
  "Independently verify that the batch's actual baking temperature stays
  within the product's required range [min,max]. Both bounds are inclusive.
  Baking too cool risks an undercooked/unsafe product; too hot risks
  burning and quality failure. Both are HARD limits."
  [actual-temp-c min-temp-c max-temp-c]
  (or (< actual-temp-c min-temp-c)
      (> actual-temp-c max-temp-c)))

(defn baking-time-exceeded?
  "Independently verify that the batch's actual baking time does not exceed
  the product's maximum. Time zero is oven-entry; time is recorded
  continuously via the oven controller log."
...
(defn moisture-out-of-target?
  ...
  within tolerance of the product's target moisture."
...
(defn scale-calibration-overdue?
  "Independently verify that the mixing/dough scale was calibrated within
  the last 180 days. `last-calibration-epoch-ms` and `now-epoch-ms` are both
...
(defn weight-variance-excessive?
  ...
  Excessive variance indicates the mixing scale is out of calibration or
  the formulation was measured incorrectly."
...
(defn allergen-label-risk?
  ...
  food-safety hazard for allergic consumers). Declaring MORE allergens than
  the formulation actually contains is conservative and never a risk."
