(ns bakeryops.governor
  "Bakery Governor -- the independent compliance layer that earns
  the BakeryOpsAdvisor the right to commit. The LLM has no notion of:
    - Whether a batch's baking temperature stayed within safe range
    - Whether baking time exceeded limits for the product
    - Whether moisture content falls within tolerance
    - Whether mixing scale calibration is current
    - Whether final product weight variance is acceptable
    - Whether allergen labeling is complete and accurate
  ...
  Unlike direct baking-line control (NEVER done by this actor -- oven
  operation and dough handling remain exclusive to plant staff), the
  Governor operates on batch metadata: provenance, formulation, baking
  parameters, sanitation records, and food-safety flags. This is plant-
  operations coordination, not process control.

  CRITICAL: Any proposal involving food-safety concerns (allergen mislabeling,
  ...
  Hard violations (always HOLD, no override):
    ...
    3. Baking temperature out of range (product safety)
    4. Baking time exceeded (product safety)
    5. Moisture out of target range (product quality/safety)
  ...
  This design mirrors `refining.governor` and `meatprocessing.governor` but
  specializes on bakery-specific concerns: formulation, time-at-temperature,
  moisture, allergen accuracy, and sanitation — rather than pressure safety
  or cold-chain management."
  (:require [bakeryops.facts :as facts]
    ...
    (let [b (store/production-batch st subject)]
      (when-not (and b
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(formulation/baking-log/temperature-log/moisture-test等)が充足していない状態での提案"}]))))

(defn- baking-temp-out-of-range-violations
  "For `:log-production-batch`, INDEPENDENTLY verify the batch's actual
  baking temperature stays inside its safe range via
  `registry/baking-temp-out-of-range?`. Evaluated UNCONDITIONALLY."
  [{:keys [op subject]} st]
  (when (= op :log-production-batch)
    (let [b (store/production-batch st subject)
          p (when b (facts/product-type-by-id (:product-type b)))]
      (when (and b p (:baking-temp-c b)
                 (registry/baking-temp-out-of-range?
                  (:baking-temp-c b)
                  (:baking-temp-c-min p)
                  (:baking-temp-c-max p)))
        [{:rule :baking-temp-out-of-range
          :detail (str subject " の焼成温度(" (:baking-temp-c b) " ℃)が安全窓["
                      (:baking-temp-c-min p) ", "
                      (:baking-temp-c-max p) "] ℃ の外 -- バッチ登録提案は進められない")}]))))

(defn- baking-time-exceeded-violations
  "For `:log-production-batch`, INDEPENDENTLY verify that the batch's
  baking time does not exceed the maximum via `registry/baking-time-exceeded?`.
  Evaluated UNCONDITIONALLY."
  [{:keys [op subject]} st]
  (when (= op :log-production-batch)
    (let [b (store/production-batch st subject)
          p (when b (facts/product-type-by-id (:product-type b)))]
      (when (and b p (:baking-time-minutes b)
                 (registry/baking-time-exceeded?
                  (:baking-time-minutes b)
                  (:baking-time-max-minutes p)))
        [{:rule :baking-time-exceeded
          :detail (str subject " の焼成時間(" (:baking-time-minutes b)
                      " 分)が限度(" (:baking-time-max-minutes p)
...
(defn- moisture-out-of-target-violations
  ...
  moisture falls within tolerance via `registry/moisture-out-of-target?`."
  [{:keys [op subject]} st]
  (when (= op :log-production-batch)
    (let [b (store/production-batch st subject)
          p (when b (facts/product-type-by-id (:product-type b)))]
...
(defn- food-safety-flag-unresolved-violations
  ...
  Food-safety concerns (suspected allergen cross-contact, contamination,
  unexpected defects) raised during production or inspection MUST be resolved
